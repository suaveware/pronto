...
<div class="inline-flex flex-col w-full {$$props.class}">
	{#if label}
		<label class="block uppercase text-blueGray-600 text-xs font-bold mb-2" for="form-select">
			{label}
		</label>
	{/if}
	<div
	...
	>
		...
		<div
			class="absolute box-border bg-white top-full rounded translate-y-2 left-0 w-full shadow z-30 transition-all duration-75 {open
				? 'opacity-100'
				: 'hidden opacity-0'}"
		>
			{#each options as option (option.value)}
				<div
					class="p-2 box-border hover:text-white pointer-events-auto hover:bg-teal-500 {value ===
					option.value
